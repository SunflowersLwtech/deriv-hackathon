...
graph TB
    ...
    subgraph AI_Engine["AI Engine"]
        ORCH["Agent Router - DeepSeek Function Calling"]
...
| AI Orchestration | DeepSeek Tool Use (native) | Lightweight function-call routing via DeepSeek's function calling — no heavy framework overhead, faster to debug than LangGraph, sufficient for hackathon scope |
| Primary LLM | DeepSeek-V3.2 | Cost-effective, excellent Chinese support, function calling support |
| Database | Supabase (PostgreSQL) | Free tier, built-in auth, realtime subscriptions, instant setup |
| Channel Layer | InMemoryChannelLayer | Simple, sufficient for single-process demo deployment |
| Deployment | Vercel + Railway | Fast deploy, free tiers, WebSocket support |

> **Why DeepSeek Function Calling over LangGraph?** LangGraph adds multi-agent state machine complexity that increases debugging time significantly during a hackathon. DeepSeek's function calling provides the same routing capability (market question → market tools, behavior question → behavior tools) with a simple function dispatch pattern. We can always upgrade to LangGraph post-hackathon if needed.
...
### Simplified Agent Routing via DeepSeek Function Calling

```mermaid
graph TB
    USER["User Query / Event Trigger"]

    subgraph Router["DeepSeek Function Calling Router"]
        DEEPSEEK_MAIN["DeepSeek API - Single Entry Point"]
        TOOLS["Registered Tool Functions"]
        COMPLY["Compliance Guard Layer"]
        DEEPSEEK_MAIN --> COMPLY
        COMPLY --> TOOLS
    end
...
> **How it works:** We register all tools (market, behavior, content) as DeepSeek function definitions in a single API call. DeepSeek's function calling decides which tools to invoke based on the user query. A Django service layer dispatches the actual function calls. No LangGraph state machine needed — just clean Python functions behind DeepSeek's routing.
...
| Supabase | Free | Free |
| DeepSeek API | Pay-per-use | ~$1-2 for demo |
...
sequenceDiagram
    ...
    participant AI as DeepSeek Function Calling
    ...
    TQ->>AI: DeepSeek selects market tools
...
Django Channels (WebSocket support with InMemoryChannelLayer)
OpenAI Python SDK (DeepSeek API - OpenAI compatible)
atproto Python SDK (Bluesky publishing)
```

### AI/ML
```
DeepSeek-V3.2 API with Function Calling (primary LLM + routing)
...
│   │   ├── serializers.py
│   │   ├── urls.py
│   │   └── tools.py            # DeepSeek tool functions for market
│   ├── behavior/               # Behavioral coaching app
│   │   ├── models.py
│   │   ├── views.py
│   │   ├── serializers.py
│   │   ├── urls.py
    │   │   ├── tools.py            # DeepSeek tool functions for behavior
...
│   │   ├── serializers.py
│   │   ├── urls.py
    │   │   ├── tools.py            # DeepSeek tool functions for content
│   │   ├── personas.py         # Persona voice configs
│   │   └── bluesky.py          # Bluesky AT Protocol client (post + thread)
    │   ├── agents/                 # AI agent layer
    │   │   ├── router.py           # DeepSeek Function Calling dispatcher
    │   │   ├── llm_client.py       # Unified DeepSeek client
    │   │   ├── prompts.py          # All system prompts + compliance rules
    │   │   ├── tools_registry.py   # Tool definitions for DeepSeek API
...
| Deriv API rate limits | Batch requests, fallback to cached data, request throttling |
...
| AI routing complexity | Use DeepSeek function calling instead of LangGraph — simpler to debug, no state machine overhead |
...
| WebSocket scalability | InMemoryChannelLayer sufficient for demo; can upgrade to Redis for production if needed |
...
| AI Orchestration | LangGraph state machine | DeepSeek Function Calling | Simpler to debug, less overhead, sufficient for hackathon |
